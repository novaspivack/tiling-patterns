...
def test_encode_decode_round_trips_for_random_tables(num_states: int) -> None:
    table = random_table(num_states, random.Random(num_states))
    code = encode_rule(num_states, table)
    decoded_states, decoded_table = decode_rule(code)
